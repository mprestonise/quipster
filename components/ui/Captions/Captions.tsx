...
export default function Captions(userid: Record<string, unknown> | undefined) {
    const [photo, setPhoto] = useState({ url: '' });
    const [caption, setCaption] = useState<string | undefined>(undefined);

    console.log('What is userid in Captions?', userid?.id);
    ...
    <div className="max-w-2xl m-auto mt-5 text-sm sm:text-center sm:text-xs">
        ...
        <Uploader userid={userid?.id} updatePhotoURL={setPhoto} />
